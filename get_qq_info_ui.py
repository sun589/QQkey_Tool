    ...
    def setupUi(self, Dialog):
        ...
        self.pushButton.clicked.connect(lambda: (self.textEdit.setText(""), Thread(
            target=self.generate_trojan).start()) if not self.func_running else QtWidgets.QMessageBox.critical(Dialog,
                                                                                                           "错误",
                                                                                                           "已有功能正在运行,请等待运行完毕!")
            if self.icon_path == "" and os.path.isfile(self.icon_path) else (QtWidgets.QMessageBox.information(self.centralwidget,"提示","检测到图标文件不存在,将使用默认图标!"),self.generate_trojan(reset_icon_path=True)))
